...
位置选择样式可选“单一最近地物”或“自定义范围汇总”。范围输入默认200、500米，可自由增加、删除和修改，支持小数，以英文/中文逗号分隔，例如100、300、800。只接受正有限数值，重复范围自动去重并从大到小排序。每个范围自动新增“邻近路网（对应米数）”字段，汇总范围内所有地物类别，以顿号分隔、去重，无匹配留空。范围汇总独立于最大距离阈值及方位/距离描述开关。


勾选“输出地块中心经纬度”后，计算米制分析投影下的面积重心并转换为 WGS84 十进制度，保留8位小数；凹面或多部件面的中心可能位于面外。Shapefile 字段为 center_lon、center_lat；范围字段自动使用 near_ 加米数（小数点替换为p），超过10字节时使用短字段名，附中文字段映射。
