...
位置选择样式可选“单一最近地物”或“双范围位置（200米 / 500米）”。双范围新增“邻近路网（500米）”“邻近路网（200米）”两列，分别汇总整个图斑最短距离在对应半径内的所有地物类别（含铁路、村庄），以顿号分隔并去重，例如“县道、乡道、村庄”。没有匹配时留空，不带方位、距离或“旁”字。两个范围独立于原最大距离阈值；500米包含200米范围。

勾选“输出地块中心经纬度”后，计算米制分析投影下的面积重心并转换为 WGS84 十进制度，保留8位小数；凹面或多部件面的中心可能位于面外。Shapefile 字段为 center_lon、center_lat；双范围字段为 near_200 和 near_500，附中文字段映射。
